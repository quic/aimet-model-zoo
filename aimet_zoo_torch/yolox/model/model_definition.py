...
#  @@-COPYRIGHT-END-@@
# =============================================================================
"""Class for downloading and setting up of optmized and original yolox model for AIMET model zoo"""
...
class YOLOX(Downloader):
    ...
    def __init__(self, model_config: str = None):
        ...
        if model_config:
            config_filepath = os.path.join(
                parent_dir, "model_cards", f"{model_config}.json"
            )
    ...
    def from_pretrained(self, quantized=False):
        ...
        if quantized:
            ...
            state_dict = torch.load(
                self.path_post_opt_weights, map_location=self.device
            )
    ...
    def get_quantsim(self, quantized=False):
        ...
        if self.cfg["name"].endswith("s"):
            ...
            raise ValueError(
                "Currently only YOLOX-s (small) and YOLOX-l (large) model are allowed."
            )
