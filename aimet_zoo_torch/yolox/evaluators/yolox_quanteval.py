...
def read_model_configs_from_model_card(model_card):
    ...
    config_filepath = os.path.join(parent_dir, 'model', 'model_cards', f'{model_card}.json')
    ...
    with open(config_filepath) as f_in:
        cfg = json.load(f_in)
        input_shape = tuple(x if x != None else 1 for x in cfg['input_shape'])
...
def arguments():
    ''' argument parser '''
    parser = argparse.ArgumentParser(description='Evaluation script for PyTorch YOLOX models.')
    parser.add_argument('--model-config',       help='Select the model configuration', type=str, default="yolox_s", choices=["yolox_s", "yolox_l"])
    parser.add_argument('--dataset-path',       help='Path to COCO2017 parent folder containing val2017', type=str, required=True)
...
def main(args):
    ...
    input_shape, default_param_bw = read_model_configs_from_model_card(args.model_config)
    ...
    print(f'Original Model | {default_param_bw}-bit Environment | mAP: {100*mAP_orig_int8:.2f}%')
    print(f'Optimized Model | {default_param_bw}-bit Environment | mAP: {100*mAP_optim_int8:.2f}%')
