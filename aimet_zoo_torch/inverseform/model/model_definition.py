...
#  @@-COPYRIGHT-END-@@
# =============================================================================
"""Class for downloading and setting up of optmized and original inverseform model for AIMET model zoo"""
...
from aimet_zoo_torch.inverseform.model.utils.config import assert_and_infer_cfg, cfg
from aimet_zoo_torch.inverseform.model.models.lighthrnet import HRNet16
from aimet_zoo_torch.inverseform.model.models.ocrnet import OCRNet


class HRNetInverseForm(Downloader):
    """HRNetInverseForm parent class with automated loading of weights and providing a QuantSim with pre-computed encodings"""
    def __init__(self, model_config: str = None, num_classes=19):
        ...
        if "16" in model_config:
            assert_and_infer_cfg(
                result_dir="", global_rank=0, syncbn=False, hrnet_base=16
            )
            C = cfg
            C.MODEL.HR18 = False
            C.MODEL.HR16 = True
            C.MODEL.DOWN_CONV = True
            self.model = HRNet16(num_classes=num_classes, criterion=None)
        elif "48" in model_config:
            assert_and_infer_cfg(
                result_dir="", global_rank=0, syncbn=False, hrnet_base=48
            )
            C = cfg
            C.MODEL.HR18 = False
            C.MODEL.HR16 = False
            C.MODEL.DOWN_CONV = False
            self.model = OCRNet(num_classes=num_classes, criterion=None)
