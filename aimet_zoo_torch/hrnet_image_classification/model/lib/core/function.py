...
def validate(config, val_loader, model, criterion, output_dir, tb_log_dir,
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            ############ Raw file validation ###############
            path = path[0].split('/')[-1][:-5]
            #print("\nPath = ",path)

            preds = []
            rootdir = "/local/mnt/workspace/selventh/QNN_Binaries/hrnet_w32_img_cls/50K_Raw_QC_224/" #Path of Raw files
            #rootdir = "/local/mnt/workspace/selventh/QNN_Binaries/hrnet_w32_img_cls/QC_2000_raw_hrnetw32/"
            for subdir, dirs, files in os.walk(rootdir):
                for file in files:
                    preds.append(os.path.join(subdir, file))
            path = str(path)+'.raw'
            for str1 in preds:
                if path in str1:
                    rawpath = str1
            #print(rawpath)
            sample = np.fromfile(rawpath, dtype = np.float32)
            sample = torch.tensor(sample).to('cuda')
            sample = torch.reshape(sample,(1,224,224,3))
            input1 = sample.permute(0,3,1,2)
            #################################################
            '''
