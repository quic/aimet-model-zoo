...
def download_weights(prefix):
    ...
    if not os.path.exists("./default_config_per_channel.json"):
        urllib.request.urlretrieve(
            QUANTSIM_CONFIG_URL, "default_config_per_channel.json"
        )

    # Download optimized model
    if not os.path.exists(f"./{prefix}.pth"):
        urllib.request.urlretrieve(
            f"{OPTIMIZED_CHECKPOINT_URL}/{prefix}.pth", f"{prefix}.pth"
        )
    if not os.path.exists(f"./{prefix}.encodings"):
        urllib.request.urlretrieve(
            f"{OPTIMIZED_CHECKPOINT_URL}/{prefix}.encodings",
            f"{prefix}.encodings")
...
def get_imagenet_dataloader(image_dir, BATCH_SIZE=128):
    ...
    def generate_dataloader(data, transform, batch_size=32):
...
def arguments():
    ...
    parser.add_argument(
        "--fp32-model",
        help="FP32 torchvision model to load",
        default=None,
        choices=["resnet18", "resnet50", "regnet_x_3_2gf"],
        type=str,
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--batch-size",
        help="Data batch size for a model",
        type=int,
        default=128)
    parser.add_argument(
        "--config-file",
...
def main():
    ...
    # Download weights for optimized model and load optimized model and
    # encodings
    ...
    encoding_dataloader = ImageNetDataLoader(
        args.evaluation_dataset, image_size=224, num_samples_per_class=2
    )

    # Compute activation encodings
    sim.compute_encodings(
        forward_pass,
        forward_pass_callback_args=encoding_dataloader.data_loader)
    ...
    print(
        f"Optimized Model | Accuracy on {args.default_param_bw}-bit device: {quant_acc:.4f}"
    )
