...
class ViTEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.cat_1 = elementwise_ops.Concat(axis=1)
        self.cat_2 = elementwise_ops.Concat(axis=1)

        self.cls_token = nn.Parameter(torch.zeros(1, 1, config.hidden_size))
        self.identity_cls_token = elementwise_ops.Add()
        ...
        self.identity_class_pos_embed = elementwise_ops.Add()
        self.identity_patch_pos_embed = elementwise_ops.Add()
    ...
    def interpolate_pos_encoding(self, embeddings, height, width):
        ...
        class_pos_embed = self.identity_class_pos_embed(
            self.position_embeddings[:, 0], 
            torch.zeros_like(self.position_embeddings[:, 0]))
        #patch_pos_embed = self.position_embeddings[:, 1:]
        patch_pos_embed = self.identity_patch_pos_embed(
            self.position_embeddings[:, 1:],
            torch.zeros_like(self.position_embeddings[:, 1:]))
        ...
        return self.cat_1(class_pos_embed.unsqueeze(0), patch_pos_embed)
    ...
    def forward(self, pixel_values, interpolate_pos_encoding=False):
        ...
        cls_tokens = self.identity_cls_token(
            self.cls_token.expand(batch_size, -1, -1),
            torch.zeros_like(self.cls_token.expand(batch_size, -1, -1))
        )
        #embeddings = torch.cat((cls_tokens, embeddings), dim=1)
        embeddings = self.cat_2(cls_tokens, embeddings)

        # add positional encoding to each token
        if interpolate_pos_encoding:
            embeddings = self.add_1(embeddings, self.interpolate_pos_encoding(embeddings, height, width))
        else:
            embeddings = self.add_1(embeddings, self.position_embeddings)
...
class ViTOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        ...
        hidden_states = self.add_1(hidden_states, input_tensor)
...
class ViTLayer(nn.Module):
    ...
    def forward(self, hidden_states, head_mask=None, output_attentions=False):
        ...
        hidden_states = self.add_1(attention_output, hidden_states)
