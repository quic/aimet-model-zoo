...
to aimet_zoo_torch/Docs/SRGAN.md
"""
...
def evaluate_generator(
    ...
    for data in test_loader:
        need_GT = not test_loader.dataset.opt["dataroot_GT"]
...
def setup_mmsr_configuration(args):
    ...
    if isEmpty(args.dataset_path + "/Set14"):
        raise ValueError(
            "Set14 dataset not exist or empty, please download dataset and put to correct path!"
        )
    if isEmpty(args.dataset_path + "/Set5"):
        raise ValueError(
            "Set5 dataset not exist or empty, please download dataset and put to correct path!"
        )
    if isEmpty(args.dataset_path + "/BSD100_SR"):
        raise ValueError(
            "BSD dataset not exist or empty, please download dataset and put to correct path!"
        )
    ...
    # configure test_SRGAN.yml in current directory and configure yml file for
    # mmsr
        ...
        for index, line in enumerate(lines):
            ...
            if "dataroot_GT" in line and line.endswith("Set5\n"):
                ...
                sources.write(
                    ...
                    + ": "
                    + args.dataset_path
                    + "/Set14/image_SRF_4_LR\n"
                )
            elif "pretrain_model_G" in line:
                sources.write(
                    "  pretrain_model_G" + ": " + os.getcwd() + "/MSRGANx4.pth"
                )
            else:
                sources.write(line)
...
def parse_args():
    ...
    parser.add_argument(
        "--dataset-path",
        help="path to data set that includes Set14, Set5 and BSD100 folder",
        default="./dataset/",
        type=str,
    )
    parser.add_argument(
        "--mmsr-path",
        help="path to patched mmsr github repo",
        default="./mmsr/",
        type=str,
    )
    parser.add_argument(
