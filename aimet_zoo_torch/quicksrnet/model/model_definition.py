...
class QuickSRNet(QuickSRNetBase, Downloader):
    ...
    def __init__(self, model_config = None, num_channels=16, num_intermediate_layers=3, scaling_factor=2, use_ito_connection=False, **kwargs):
        """
        ...
        QuickSRNetBase.__init__(self, scaling_factor = self.cfg['model_args']['scaling_factor'] if self.cfg else scaling_factor,
                             num_channels = self.cfg['model_args']['num_channels'] if self.cfg else num_channels,
                             num_intermediate_layers = self.cfg['model_args']['num_intermediate_layers'] if self.cfg else num_intermediate_layers,
                             use_ito_connection = self.cfg['model_args']['use_ito_connection'] if self.cfg else use_ito_connection,
                             **kwargs)
