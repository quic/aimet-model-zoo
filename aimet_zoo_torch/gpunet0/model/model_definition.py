...
#  @@-COPYRIGHT-END-@@
# =============================================================================
"""Class for downloading and setting up of optmized and original gpunet0 model for AIMET model zoo"""
...
class GPUNet0(Downloader):
    ...
    def from_pretrained(self, quantized=False):
        ...
        if quantized:
            if self.url_post_opt_weights and not os.path.exists(
            ...
            if self.url_pre_opt_weights and not os.path.exists(
    ...
    def get_quantsim(self, quantized=False):
        ...
        if quantized:
            if self.url_aimet_encodings and not os.path.exists(
                    self.path_aimet_encodings
            ):
                self._download_aimet_encodings()
            if self.url_adaround_encodings and not os.path.exists(
