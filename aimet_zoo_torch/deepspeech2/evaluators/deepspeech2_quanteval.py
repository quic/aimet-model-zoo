...
def run_quantsim_evaluation(args):
    ...
    deepspeech_pytorch.model.DeepSpeech.infer = (
        deepspeech_pytorch.model.DeepSpeech.forward
    )
    ...
    print(
        f"Original Model | {args.default_output_bw}-bit Environment | Average WER {wer:.4f}"
    )
    ...
    wer, cer, output_data = eval_func(sim.model, None)
    print(
        f"Optimized Model | {args.default_output_bw}-bit Environment | Average WER {wer:.4f}"
    )
...
def arguments():
    ...
    parser.add_argument(
        "--test-manifest",
        help="Path to csv to do eval on.",
        type=str,
