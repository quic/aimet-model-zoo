...
"""
PyTorch Huggingface DistilBERT model redefined with elementwise/functionals replaced with class definitions
"""
...
class Embeddings(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        embeddings = self.add(word_embeddings, position_embeddings)  # (bs, max_seq_length, dim)
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask, head_mask=None, output_attentions=False):
        ...
        scores = self.matmul_1(q, k.transpose(2, 3))  # (bs, n_heads, q_length, k_length)
        mask = (mask == 0).view(mask_reshp).expand_as(scores)  # (bs, n_heads, q_length, k_length)
        
        #mask_ = mask * (-10000) # Mul_35 constant mul 1
        mask_ = mask * (-6)
        scores = self.mask_add(scores, mask_)
        ...
        if head_mask is not None:
            weights = weights * head_mask  # not on path / constant mul , head_mask will be zero and ones

        context = self.matmul_2(weights, v)  # (bs, n_heads, q_length, dim_per_head)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, attn_mask=None, head_mask=None, output_attentions=False):
        ...
        sa_output = self.add_1(sa_output, x)
        sa_output = self.sa_layer_norm(sa_output)  # (bs, seq_length, dim)

        # Feed Forward Network
        ffn_output = self.ffn(sa_output)  # (bs, seq_length, dim)
        ffn_output = self.add_2(ffn_output, sa_output)
