...
import argparse
from aimet_zoo_torch.abpn import ABPN
from aimet_zoo_torch.common.super_resolution.psnr import evaluate_average_psnr
from aimet_zoo_torch.common.super_resolution.utils import load_dataset, pass_calibration_data
...
def arguments():
    ...
    parser.add_argument('--dataset-path', help='path to image evaluation dataset', type=str, required=True)
    parser.add_argument('--model-config', help='model configuration to be tested', type=str, required=True)
...
def main():
    ...
    model_fp32 = ABPN(model_config = args.model_config)
    model_fp32.from_pretrained(quantized=False)
    sim_fp32 = model_fp32.get_quantsim(quantized=False)

    model_int8 = ABPN(model_config = args.model_config)
