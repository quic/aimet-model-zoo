...
from aimet_zoo_torch.common.super_resolution.models import ABPNRelease


class ABPN(ABPNRelease, Downloader):
    """ABPN parent class with automated loading of weights and providing a QuantSim with pre-computed encodings"""
    def __init__(self, model_config=None, num_channels=28, scaling_factor=2, **kwargs):
        """
        ...
        self.scaling_factor = self.cfg['model_args']['scaling_factor'] if self.cfg else scaling_factor
        self.num_channels = self.cfg['model_args']['num_channels'] if self.cfg else num_channels
        if self.cfg:
        ...
        ABPNRelease.__init__(self, scaling_factor = self.cfg['model_args']['scaling_factor'] if self.cfg else scaling_factor,
                             num_channels = self.cfg['model_args']['num_channels'] if self.cfg else num_channels,
                             **kwargs)
