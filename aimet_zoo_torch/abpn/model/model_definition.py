...
#  @@-COPYRIGHT-END-@@
# =============================================================================
"""Class for downloading and setting up of optmized and original ABPN model for AIMET model zoo"""
#pylint:disable = import-error
#ignoring this due to aimet not setup in environment
import os
import json
import torch
...
class ABPN(ABPNRelease, Downloader):
    ...
    def __init__(self, model_config=None, num_channels=28, scaling_factor=2, **kwargs):
        ...
        self.scaling_factor = (
            self.cfg["model_args"]["scaling_factor"] if self.cfg else scaling_factor
        )
        self.num_channels = (
            self.cfg["model_args"]["num_channels"] if self.cfg else num_channels
        )
        if self.cfg:
            Downloader.__init__(
                self,
