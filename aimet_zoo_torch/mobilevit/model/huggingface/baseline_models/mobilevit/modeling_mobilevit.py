...
# version mismatch
#from transformers.modeling_utils import fined_pruneable_heads_and_indices, prune_linear_layer
...
from aimet_zoo_torch.mobilevit.model.huggingface import elementwise_ops as helementwise_ops
    ...
    def __init__(
        ...
        if use_activation:
            if isinstance(use_activation, str):
                ...
                self.activation = activation.get_activation(use_activation)
            elif isinstance(config.hidden_act, str):
                #self.activation = ACT2FN[config.hidden_act]
                self.activation = activations.get_activation(config.hidden_act)
            else:
...
class MobileViTInvertedResidual(nn.Module):
    ...
    def __init__(
        ...
        self.skip_connection = elementwise_ops.Add()
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_residual:
            return self.skip_connection(residual, features)
        else:
            return features
...
class MobileViTSelfAttention(nn.Module):
    def __init__(self, config: MobileViTConfig, hidden_size: int) -> None:
        ...
        self.matmul_qk = elementwise_ops.MatMul()
        self.matmul_v = elementwise_ops.MatMul()
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        context_layer = self.matmul_v(attention_probs, value_layer)
...
class MobileViTIntermediate(nn.Module):
    def __init__(self, config: MobileViTConfig, hidden_size: int, intermediate_size: int) -> None:
        ...
        if isinstance(config.hidden_act, str):
            ...
            self.intermediate_act_fn = activations.get_activation(config.hidden_act)
        else:
...
class MobileViTOutput(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor) -> torch.Tensor:
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.skip_connection(hidden_states, input_tensor)
        return hidden_states
...
class MobileViTTransformerLayer(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = self.skip_connection(attention_output, hidden_states)
...
class MobileViTLayer(nn.Module):
    ...
    def __init__(
        ...
        self.concat = elementwise_ops.Concat(axis=1)
        self.interpolate_fold = helementwise_ops.Interpolate(mode="bilinear", align_corners=False)
        self.interpolate_unfold = helementwise_ops.Interpolate(mode="bilinear", align_corners=False)
        self.add_tmp = elementwise_ops.Add()
    ...
    def unfolding(self, features: torch.Tensor) -> Tuple[torch.Tensor, Dict]:
        ...
        if new_width != orig_width or new_height != orig_height:
            ...
            #)
            # make size tensors to keep track 
            size = torch.tensor([new_height, new_width], dtype=torch.int)
            features = self.interpolate_unfold(features, size)
            interpolate = True
    ...
    def folding(self, patches: torch.Tensor, info_dict: Dict) -> torch.Tensor:
        ...
        if info_dict["interpolate"]:
            ...
            #)
            size = torch.tensor(list(info_dict["orig_size"]), dtype=torch.int)
            features = self.interpolate_fold(features, size)
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        ...
        features = self.concat(residual, features)
