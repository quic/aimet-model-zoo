...
def arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="mobilenetv2_w8a8",
        choices=["mobilenetv2_w8a8"],
    )
    parser.add_argument(
        "--dataset-path", help="Imagenet eval image", default="./ILSVRC2012/", type=str
    )
    parser.add_argument(
        "--default-output-bw",
    ...
    parser.add_argument(
        "--batch-size", help="Data batch size for a model", type=int, default=16
    )
    parser.add_argument(
...
def main():
    ...
    print(
        f"Original Model | Accuracy on {args.default_param_bw}-bit device: {orig_acc_int8:.4f}"
    )
    print(f"Optimized Model | Accuracy on 32-bit device: {optim_acc_fp32:.4f}")
    print(
        f"Optimized Model | Accuracy on {args.default_param_bw}-bit device: {optim_acc_int8:.4f}"
    )
