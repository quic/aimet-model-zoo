...
#  @@-COPYRIGHT-END-@@
# =============================================================================
""" modules for getting dataloader and eval functions"""
...
def make_dataloader(dataset_path, image_size, batch_size=16, num_workers=8):
    """
    getting dataloder from dataset path
    """
    data_loader_kwargs = {
        "worker_init_fn": work_init,
        "num_workers": min(num_workers, batch_size // 2),
    }
...
def get_dataloaders_and_eval_func(imagenet_path, image_size=224):
    """getting imagnet dataloader and evaluation function"""
    train_loader = make_dataloader(dataset_path=imagenet_path, image_size=image_size)
    val_loader = make_dataloader(dataset_path=imagenet_path, image_size=image_size)
    ...
    def eval_func(model, args):
        ...
        total_num = 0
        model.to(device)
        for _, (sample, label) in enumerate(tqdm(val_loader)):
            ...
            if total_num >= num_samples:
                break
        avg_acc = top1_acc * 100.0 / total_num
        return avg_acc
